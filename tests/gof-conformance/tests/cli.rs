...
fn write_executable_package(root: &Path) -> std::path::PathBuf {
    let package_root = root.join("executable_package");
    fs::create_dir_all(package_root.join("src")).expect("package source root should exist");
    fs::write(
        package_root.join("gof.mod"),
...
#[test]
fn gof_run_executes_http_patch_report_example_against_fake_api() {
    let example = gof_conformance::workspace_root()
        .join("examples")
        .join("http_patch_report.gof");
    let observed_requests = Arc::new(Mutex::new(Vec::<String>::new()));
    let listener = TcpListener::bind("127.0.0.1:0").expect("listener should bind");
    let address = listener.local_addr().expect("listener addr should exist");
    let observed_requests_thread = observed_requests.clone();
    ...
    gof_command()
        .arg("run")
        .arg(example)
        .env("GOF_HTTP_PATCH_BASE", format!("http://{address}"))
        .assert()
        .success()
        .stdout(predicate::str::contains("Result.Ok(value: 220)"));
    ...
    assert!(
        requests
            .iter()
            .any(|request| request.contains("PATCH /inspect HTTP/1.1")),
        "expected PATCH request, got {requests:?}"
    );
    assert!(
        requests
            .iter()
    ...
    assert!(
        requests
            .iter()
            .any(|request| request.contains("X-Trace-Id: trace-8")),
        "expected custom trace header, got {requests:?}"
    );
    assert!(
        requests
            .iter()
            .any(|request| request.contains("patched")),
        "expected PATCH payload, got {requests:?}"
    );
}
...
fn gof_run_executes_http_json_client_example_against_fake_api() {
    ...
    let server = thread::spawn(move || {
        for _ in 0..2 {
            ...
            let body = if path.contains("/health") {
                "{\"service\":\"alpha\",\"count\":7}".to_string()
    ...
    gof_command()
        .arg("run")
        .arg(example)
        .env("GOF_HTTP_JSON_BASE", format!("http://{address}"))
        .assert()
        .success()
        .stdout(predicate::str::contains("Result.Ok(value: 43)"));
    ...
    assert!(
        requests
            .iter()
            .any(|request| request.contains("GET /health HTTP/1.1")),
        "expected GET health request, got {requests:?}"
    );
    assert!(
        requests
            .iter()
            .any(|request| request.contains("POST /jobs HTTP/1.1")),
        "expected POST jobs request, got {requests:?}"
    );
    assert!(
        requests
            .iter()
    ...
    assert!(
        requests
            .iter()
            .any(|request| request.contains("X-Trace-Id: trace-9")),
        "expected trace header, got {requests:?}"
    );
    assert!(
        requests.iter().any(|request| {
            request.contains("{\"job\":\"sync\"}")
                && request.contains("Content-Type: application/json")
        }),
        "expected JSON jobs payload, got {requests:?}"
    );
}
...
#[test]
fn gof_test_keeps_passing_target_output_captured_without_nocapture() {
    let temp = tempdir().expect("tempdir should exist");
    let tests_root = temp.path().join("tests");
    let runtime_root = tests_root.join("runtime");
    fs::create_dir_all(&runtime_root).expect("runtime root should exist");
    fs::write(
        tests_root.join("nocapture_test.gof"),
        "import testing\n\ntest fn visible_case(t: TestContext):\n    print(\"lang-output\")\n    t.true(true, \"expected success\")\n",
    )
    ...
    gof_command()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("lang-output").not())
        .stdout(predicate::str::contains("doc-output").not())
        .stdout(predicate::str::contains("fixture-output").not())
        .stdout(predicate::str::contains("package-output").not())
        .stdout(predicate::str::contains("test result: 4 passed; 0 failed"));
}

#[test]
fn gof_test_nocapture_surfaces_captured_output_across_target_kinds() {
    let temp = tempdir().expect("tempdir should exist");
    let tests_root = temp.path().join("tests");
    let runtime_root = tests_root.join("runtime");
    fs::create_dir_all(&runtime_root).expect("runtime root should exist");
    ...
    gof_command()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("lang-output"))

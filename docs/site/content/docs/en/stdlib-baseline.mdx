...
- `import http` now exposes JSON request helpers such as `request_json_headers()`, `get_json(url)`, `post_json(url, body)`, and `request_json_report(...)`
...
Use the raw `http_request(...)` path when you need custom header control beyond the common JSON client flow.

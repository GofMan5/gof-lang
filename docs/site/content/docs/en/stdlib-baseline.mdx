...
<Callout type="info">
  Blocking I/O returns `Result[..., RuntimeError]` instead of hiding failure in ambient runtime state.
    ...
    The shipped `io` and `net` wrappers now also expose a `with_timeout(...)` path for the common single-budget service flow, but they still return `Result[...]` because timeout setup is fallible.
</Callout>

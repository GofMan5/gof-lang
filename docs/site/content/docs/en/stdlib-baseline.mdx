...
fn main() -> Result[int, RuntimeError]:
    deadline = deadline_after(1000)?
        listener = listen_tcp_loopback(0)?.with_deadline(deadline)
    address = listener.local_addr()?
        client = address.connect_tcp_with_control(deadline, timeout_token(1000))?
...
<Callout type="info">
  Blocking I/O returns `Result[..., RuntimeError]` instead of hiding failure in ambient runtime state.
    ...
    The shipped `net` helpers now also keep local loopback bind/connect flows typed instead of rebuilding address strings by hand.
</Callout>

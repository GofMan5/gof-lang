...
Use the raw `http_request(...)` or `request_json_report...` paths when you need request shapes beyond what the shipped header builders cover or when you want to inspect non-2xx responses before promoting them into `RuntimeError.HttpStatus(...)`.

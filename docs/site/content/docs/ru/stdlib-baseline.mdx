...
`io` и `net` теперь также ship-ят Result-returning timeout wrapper-ы и timeout-armed constructor-ы вроде `open_write_stream_with_timeout(...)`, `listener.with_timeout(...)` и `listen_tcp_loopback_with_timeout(...)`, чтобы не повторять `deadline_after(...)` и `.with_deadline(...)` в каждом локальном service path.
...
fn main() -> Result[int, RuntimeError]:
    ...
    mut client = address.connect_tcp_with_timeout(1000)?
...
Когда нужен request shape шире, чем покрывают shipped header helper-ы, или когда надо сначала вручную осмотреть non-2xx ответ, оставайся на raw `http_request(...)` / `request_json_report...` path.

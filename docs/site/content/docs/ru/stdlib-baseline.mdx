...
- `import http` — JSON request helper-ы вроде `request_json_headers()`, `get_json(url)`, `post_json(url, body)`, `request_json_report(...)`
...
Когда нужен полный контроль над custom headers, оставайся на raw `http_request(...)` path.

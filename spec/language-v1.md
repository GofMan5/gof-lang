...
- the shipped `io` module currently exposes `ReadStream`, `WriteStream`, `open_read_stream`, `open_read_stream_with_timeout`, `open_write_stream`, `open_write_stream_with_timeout`, `ReadStream.read_string`, `ReadStream.read_exact_string`, `ReadStream.read_all_string`, `WriteStream.write_string`, `WriteStream.write_all_string`, `ReadStream.with_timeout`, and `WriteStream.with_timeout`
- the shipped `time` module currently exposes `NetDeadline`, `deadline_after`, and `deadline_at_unix_millis`
- the shipped `net` module currently exposes `DuplexStream`, `TcpListener`, `SocketAddr`, `connect_tcp`, `connect_tcp_loopback`, `connect_tcp_with_control`, `connect_tcp_with_timeout`, `connect_tcp_with_timeout_budget`, `connect_tcp_loopback_with_control`, `connect_tcp_loopback_with_timeout`, `connect_tcp_loopback_with_timeout_budget`, `listen_tcp`, `listen_tcp_with_timeout`, `listen_tcp_loopback`, `listen_tcp_loopback_with_timeout`, `DuplexStream.read_string`, `DuplexStream.read_exact_string`, `DuplexStream.read_all_string`, `DuplexStream.write_string`, `DuplexStream.write_all_string`, `DuplexStream.with_timeout`, `TcpListener.with_timeout`, `TcpListener.accept_with_timeout`, `TcpListener.accept_with_timeout_budget`, `SocketAddr.connect_tcp`, `SocketAddr.connect_tcp_with_control`, `SocketAddr.connect_tcp_with_timeout`, and `SocketAddr.connect_tcp_with_timeout_budget`
...
- `open_read_stream_with_timeout(path, milliseconds)` currently accepts `(string, int)` and returns `Result[ReadStream, RuntimeError]` after opening the stream and arming its first timeout wrapper in one explicit step
- `open_write_stream(path)` currently accepts exactly one string path and returns `Result[WriteStream, RuntimeError]`
- `open_write_stream_with_timeout(path, milliseconds)` currently accepts `(string, int)` and returns `Result[WriteStream, RuntimeError]` after opening the stream and arming its first timeout wrapper in one explicit step
...
- `connect_tcp_with_timeout_budget(address, milliseconds)` currently accepts `(string, int)` and returns `Result[DuplexStream, RuntimeError]` after applying the same explicit timeout to connection setup and the first stream wrapper
...
- `connect_tcp_loopback_with_timeout_budget(port, milliseconds)` currently accepts `(int, int)` and returns `Result[DuplexStream, RuntimeError]` after applying the same explicit timeout to loopback dial setup and the first stream wrapper
- `listen_tcp(address)` currently accepts exactly one string address and returns `Result[TcpListener, RuntimeError]`
- `listen_tcp_with_timeout(address, milliseconds)` currently accepts `(string, int)` and returns `Result[TcpListener, RuntimeError]` after binding the listener and arming its first timeout wrapper in one explicit step
- `listen_tcp_loopback(port)` currently accepts exactly one `int` port and returns `Result[TcpListener, RuntimeError]`
- `listen_tcp_loopback_with_timeout(port, milliseconds)` currently accepts `(int, int)` and returns `Result[TcpListener, RuntimeError]` after binding the loopback listener and arming its first timeout wrapper in one explicit step
...
- `TcpListener.accept_with_timeout(milliseconds)` currently accepts a non-negative `int` and returns `Result[DuplexStream, RuntimeError]` after arming the blocking accept step with that timeout
- `TcpListener.accept_with_timeout_budget(milliseconds)` currently accepts a non-negative `int` and returns `Result[DuplexStream, RuntimeError]` after applying the same explicit timeout to the blocking accept step and the first returned stream wrapper
...
- `SocketAddr.connect_tcp_with_timeout_budget(milliseconds)` currently accepts exactly one non-negative `int` and returns `Result[DuplexStream, RuntimeError]` after applying the same explicit timeout to connection setup and the first stream wrapper
...
- `get_json_with_headers(url, headers, timeout_ms)` currently accepts `(string, dict[string], int)` and returns parsed `Result[json, RuntimeError]` for callers that want custom headers on the higher-level JSON GET path while surfacing non-2xx replies as `RuntimeError.HttpStatus(...)`
...
- `post_report(url, body, timeout_ms)` currently accepts `(string, string, int)` and returns the structured `Result[json, RuntimeError]` report for the explicit lower-level POST path
- `post_report_with_headers(url, body, headers, timeout_ms)` currently accepts `(string, string, dict[string], int)` and returns the structured `Result[json, RuntimeError]` report for callers that want custom headers on the lower-level POST path
...
- `post_json_with_headers(url, body, headers, timeout_ms)` currently accepts `(string, string, dict[string], int)` where `body` is JSON text and returns parsed `Result[json, RuntimeError]` for callers that want custom headers on the higher-level JSON POST path while surfacing non-2xx replies as `RuntimeError.HttpStatus(...)`

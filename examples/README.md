...
- `http_request_report.gof`: structured HTTP request reports with dedicated POST report helpers, explicit headers, timeout, and response metadata
...
- `http_request_report.gof` -> `Result.Ok(value: 216)` when `GOF_HTTP_REQUEST_BASE` points at a test endpoint that returns `202 Accepted`, body `accepted`, and header `X-Request-Id: req-42`; the example now also demonstrates the dedicated raw POST report helper plus declarative header-merge helpers

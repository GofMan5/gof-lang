...
- `[~]` typed fixtures through `fixture(test|module) fn`, DAG-backed injection, runtime caching, cleanup hooks, artifact-backed product fixtures under `tests/ui|runtime`, and the first `gof test --json` report schema
...
- `[~]` CP-M8-1: typed fixtures, cleanup hooks, doctests, product fixtures, and the first machine-readable reporter slice
...
- `[~]` add typed fixtures
- `[~]` add `gof test --json`; keep JUnit, property/fuzz/stress, and benchmark gates pending
